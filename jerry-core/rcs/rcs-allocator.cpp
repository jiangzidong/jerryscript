                                                  ...
                                                  and the next allocated record */
{
  ...
  if (next_record_p != NULL)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        node_p = rcs_chunked_list_get_prev (rec_set_p, node_p);
  ...
  else if (free_size != 0)
  {
    ...
    rcs_chunked_list_node_t *next_node_p = rcs_chunked_list_get_next (rec_set_p, node_p);

    while (next_node_p != NULL)
    {
      node_p = next_node_p;
      next_node_p = rcs_chunked_list_get_next (rec_set_p, node_p);
    }
